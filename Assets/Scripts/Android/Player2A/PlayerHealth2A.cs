...
public class PlayerHealth2A : MonoBehaviour
{
    ...
    {
        if ((damageenemy.CompareTag("AttackPlayer1")) && !isTakingDamage && !isDead && !MainPlayer.GetComponent<PlayerCharacter2A>().IsBlocking())
        {
            isTakingDamage = true;
            currentHealth--;
